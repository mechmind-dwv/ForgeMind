...
def generator(seed,pop=70):
    ...
    return [Hyp([rand_node(rng)]) for _ in range(pop)]
...
def evolve(seed,target,rounds=42,pop=70):
    ...
    for r in range(rounds):
        ...
        # survival score = evidence - complexity; diversity keeps search alive
        scored=[]
        for h in pool:
            score=h.support-1.15*h.failures-.18*complexity(h.p)
            scored.append((score,h))
        scored.sort(key=lambda z:z[0],reverse=True)
        elite=[h for _,h in scored[:12]]
        best=elite[0]
        history.append({
            "round":r,"support":best.support,"failures":best.failures,
            "program":canon(best.p),"pool_disagreement":disagreement(pool,x)
        })
        new=[Hyp(list(h.p),h.support,h.failures) for h in elite]
        while len(new)<pop:
            a=rng.choice(elite)
            c=list(a.p)
            if rng.random()<.45:
                c=crossover(c,rng.choice(elite).p,rng)
            if rng.random()<.9: c=mutate(c,rng)
            new.append(Hyp(c))
        pool=new
    pool.sort(key=lambda h:(h.failures,-h.support,complexity(h.p)))
    return pool[0],history
